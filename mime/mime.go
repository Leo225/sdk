...
func DeleteContentType(name string, content ...io.ReadSeeker) (contentType string, err error) {
	contentType = mime.TypeByExtension(filepath.Ext(name))
	if contentType == "" && len(content) > 0 {
		var buffer [sinffLen]byte
		n, _ := io.ReadFull(content[0], buffer[:])
		contentType = http.DetectContentType(buffer[:n])
		_, err = content[0].Seek(0, io.SeekStart)
		if err != nil {
			contentType = ""
		}
	}
